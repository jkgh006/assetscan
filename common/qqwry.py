...
class IPInfo(object):
    ...
    def __init__(self, dbname=None):
        ''' 初始化类，读取数据库内容为一个字符串，
        通过开始8字节确定数据库的索引信息'''
        self.dbname = dbname = os.path.join(os.path.dirname(os.path.abspath(__file__)),"../datas/qqwry.dat") if not dbname else dbname
        # f = file(dbname, 'r')
...
if __name__ == "__main__":
    print IPInfo.position_info("220.191.208.40").decode(encoding='UTF-8',errors='strict')
    print type(IPInfo.position_info("220.191.208.40").decode(encoding='UTF-8',errors='strict'))